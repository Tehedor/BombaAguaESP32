...
static void set_led(uint8_t state) {
    ...
    gpio_set_level(RELE_PIN, !led_state);  // Relé invertido (LOW activa relé)
}
...
static void get_current_status(uint8_t *response, uint16_t *response_len) {
    ...
    uint8_t calculated_led_state = led_state;
    ...
    if (current_mode == MODE_TIMER) {
        ...
        phase_info = calculated_led_state ? 0x00 : 0x01;
    }

    // [5] LED_STATE - usar valor calculado para mayor precisión
    response[5] = calculated_led_state;
    ...
    *response_len = 10;
    ...
    ESP_LOGI(TAG,
        ...
        time_remaining, total_phase_time, calculated_led_state, total_ms_elapsed);
}
...
void app_main(void) {
    ESP_LOGI(TAG, "═══════════════════════════════════════════════════");
    ESP_LOGI(TAG, "   Phase 30: Relé + LED - Timer + Ciclos");
    ESP_LOGI(TAG, "═══════════════════════════════════════════════════");
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << LED_PIN) | (1ULL << RELE_PIN),
