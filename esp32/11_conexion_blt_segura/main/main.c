...
static bool validate_pin_and_command(const uint8_t *data, uint16_t len, uint8_t *out_command) {
    if (len < 5) {
        ESP_LOGW(TAG, "❌ Invalid command length: %d (expected 5)", len);
        return false;
    }
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (param->write.len > 0) {
                ...
                if (validate_pin_and_command(param->write.value, param->write.len, &command)) {
                    led_state = (command & 0x01);
                    gpio_set_level(LED_PIN, led_state ? 1 : 0);
                    ESP_LOGI(TAG, "✅ LED: %s (PIN validated)", led_state ? "ON" : "OFF");
                    failed_attempts = 0;
